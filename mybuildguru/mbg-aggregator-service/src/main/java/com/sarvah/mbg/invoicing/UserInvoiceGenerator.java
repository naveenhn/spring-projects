...
import com.sarvah.mbg.invoicing.model.OrderForInvoice;
...
public class UserInvoiceGenerator {
	...
	public UserInvoiceGenerator() {

	}

	static OrderForInvoice order;
	PDDocument document;
	...
	private void addMixed(List<Tuple<String>> line) throws IOException {
	...
	private void addImage() {
		try {
			...
			contents.close();
			// addBox(Arrays.asList("RETAIL INVOICE", "RETAIL INVOICE"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private void addTotalMetaData() throws IOException, ParseException {
		...
		addRegularFontCustomForTotalSavings(Arrays.asList("TOTAL SAVINGS ", "",
	...
	public String generateInvoiceForEnduser(OrderInfoObject orderInfoObject)
			throws Exception {
		return generateInvoice(new OrderForInvoice(orderInfoObject));
	}

	private String generateInvoice(OrderForInvoice order) throws Exception {
		this.order = order;
		...
		// addDeclaration("Declaration           ,This is to declare....");
		 ...
		 File file = new File("/home/mowgli/EnduserInvoice.pdf");

//		File file = new File("EnduserInvoice.pdf");
